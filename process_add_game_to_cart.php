...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_to_cart'])) {
    ...
    // Check if the built_game_id is already in the cart for the user
    $check_query = "SELECT * FROM cart WHERE user_id = '$user_id' AND built_game_id = '$built_game_id'";
    $check_result = mysqli_query($conn, $check_query);

    if (mysqli_num_rows($check_result) > 0) {
        echo "Game is already in the cart.";
    } else {
